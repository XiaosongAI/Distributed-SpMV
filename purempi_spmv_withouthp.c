...
void writeresults(char *filename_res, char *filename, int m, int n, int nnzR, double GFlops, int processors)
{
    FILE *fres = fopen(filename_res, "a");
    if (fres == NULL) printf("Writing results fails.\n");
    fprintf(fres, "%s, %i, %i, %d, %lf GFlops, %i\n", filename, m, n, nnzR, GFlops, processors);
    fclose(fres);
}
...
int main(int argc, char ** argv)
{
    ...
    gettimeofday(&t1, NULL);
    for (int r = 0; r < NTIMES; r++)
        spmv_serial(submatrix.rownum, submatrix.rowptr, submatrix.colidx, submatrix.val, y, x); 
    gettimeofday(&t2, NULL);
    if (id == 0)
    {
	...
	writeresults("purempi_withouthp.csv", filename, sourcematrix.rownum, sourcematrix.colnum, sourcematrix.nnznum, GFlops, p);
    }
